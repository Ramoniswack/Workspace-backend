...
// @access  Private
const getMyWorkspaces = asyncHandler(async (req, res, next) => {
...
// @access  Private
const getWorkspaceAnalytics = asyncHandler(async (req, res, next) => {
...
const updateMemberCustomRole = asyncHandler(async (req, res, next) => {
    ...
    if (customRoleTitle !== null && customRoleTitle !== undefined) {
        ...
        customRoleTitle = (0, xss_1.default)(customRoleTitle);
    ...
    if (customRoleTitle) {
        ...
        if (!member.customRoleTitle) {
            ...
            const currentCustomRoleCount = workspace.members.filter((m) => m.customRoleTitle && m.customRoleTitle.trim() !== '').length;
    ...
    const updatedMember = await workspaceService.updateMemberCustomRole(workspaceId, memberId, customRoleTitle);
    res.status(200).json({
        success: true,
