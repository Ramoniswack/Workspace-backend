...
exports.getUsage = exports.checkEntitlement = void 0;
...
const checkEntitlement = async (req, res) => {
    try {
        ...
        switch (action) {
            case 'useCustomRoles':
                result = await entitlementService_1.default.canUseCustomRoles(userId);
                break;
            case 'createTable':
                result = await entitlementService_1.default.canCreateTable(userId);
                ...
                break;
            case 'addRow':
                result = await entitlementService_1.default.canAddRow(userId);
                if (!result.allowed && result.reason?.includes('limit reached')) {
                    const usage = await entitlementService_1.default.getTotalUsage(userId);
            ...
            case 'addColumn':
                ...
                result = await entitlementService_1.default.canAddColumn(userId, tableId);
                if (!result.allowed && result.reason?.includes('limit reached')) {
