...
const updateMemberRole = asyncHandler(async (req, res, next) => {
    ...
    if (workspace.owner.toString() === userId && role !== "owner") {
        return next(new AppError("Cannot change owner role. Transfer ownership first.", 400));
    }
    // Check if trying to make someone else owner
    if (role === "owner" && workspace.owner.toString() !== userId) {
        return next(new AppError("Cannot assign owner role. Use transfer ownership endpoint.", 400));
    }
    ...
    const updatedWorkspace = await Workspace.findById(workspaceId).populate("members.user", "name email");
    const updatedMember = updatedWorkspace.members.find((m) => m.user._id.toString() === userId);
    res.status(200).json({
        success: true,
...
const removeMember = asyncHandler(async (req, res, next) => {
    ...
    if (workspace.owner.toString() === userId) {
        return next(new AppError("Cannot remove workspace owner. Transfer ownership first.", 400));
    }
...
const inviteMember = asyncHandler(async (req, res, next) => {
    ...
    const existingMemberIndex = workspace.members.findIndex((m) => m.user.toString() === user._id.toString());
 ...
 */
const updateMyStatus = asyncHandler(async (req, res, next) => {
    const { workspaceId } = req.params;
