...
class ListService {
    async createList(data) {
        ...
        const isWorkspaceMember = workspace.members.some((member) => member.user.toString() === createdBy);
    ...
    async getSpaceLists(spaceId, userId) {
        ...
        console.log(`[ListService] All list IDs:`, allLists.map((l) => ({ id: l._id.toString(), name: l.name })));
        ...
        if (isOwner || isAdmin) {
            ...
            const accessibleListIds = userListMemberships.map((lm) => lm.list.toString());
            ...
            listsToReturn = allLists.filter((list) => accessibleListIds.includes(list._id.toString()));
            console.log(`[ListService] Returning ${listsToReturn.length} filtered lists`);
            console.log(`[ListService] Filtered list names:`, listsToReturn.map((l) => l.name));
        }
        // Add task counts and list members to each list
        const listsWithCounts = await Promise.all(listsToReturn.map(async (list) => {
            ...
            const memberIds = listMembers.map((lm) => lm.user.toString());
            return {
                ...list,
                taskCount,
                completedCount,
    ...
    async updateList(listId, userId, updateData) {
        ...
        if (updateData.folderId !== undefined)
            list.folderId = updateData.folderId;
        await list.save();
        // Log audit
