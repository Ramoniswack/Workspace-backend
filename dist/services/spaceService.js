...
class SpaceService {
    async createSpace(data) {
        ...
        const isMember = workspaceDoc.members.some((member) => member.user.toString() === owner);
        if (!isMember) {
    ...
    async getWorkspaceSpaces(workspaceId, userId) {
        ...
        const spacesAsMember = allSpaces.filter((space) => {
    ...
    async updateSpace(spaceId, userId, updateData) {
        ...
        if (updateData.description !== undefined)
            space.description = updateData.description;
        if (updateData.status)
            space.status = updateData.status;
        await space.save();
        // Log audit
    ...
    async deleteSpace(spaceId, userId) {
        ...
        const member = space.members.find((m) => m.user.toString() === userId);
    ...
    async addMemberToSpace(spaceId, userId, memberId, role = 'member') {
        const space = await Space.findOne({
            _id: spaceId,
            isDeleted: false
        });
        ...
        const existingMember = space.members.find((m) => m.user.toString() === memberId);
        if (existingMember) {
    ...
    async removeMemberFromSpace(spaceId, userId, memberId) {
        const space = await Space.findOne({
            _id: spaceId,
            isDeleted: false
        });
        ...
        space.members = space.members.filter((m) => m.user.toString() !== memberId);
        await space.save();
        // Log activity
        await logger.logActivity({
            userId,
    ...
    async inviteExternalUsers(spaceId, userId, invites) {
        const space = await Space.findOne({
            _id: spaceId,
            isDeleted: false
        }).populate("owner", "name email");
        ...
        const workspaceMember = workspace.members.find((m) => m.user.toString() === userId || m.user._id?.toString() === userId);
        const isWorkspaceAdmin = workspaceMember?.role === 'admin' || workspaceMember?.role === 'owner';
        if (!isWorkspaceOwner && !isWorkspaceAdmin) {
        ...
        for (const invite of invites) {
            try {
                ...
                if (existingUser) {
                    ...
                    const isInSpace = space.members.some((m) => m.user.toString() === existingUser._id.toString());
                    if (isInSpace) {
