...
class TaskService {
    async createTask(data) {
        ...
        const isMember = workspace.members.some((member) => member.user.toString() === createdBy);
        if (!isMember) {
        ...
        if (data.customFieldValues && data.customFieldValues.length > 0) {
            validatedCustomFieldValues = await customFieldService.validateCustomFieldValues(data.customFieldValues, list.workspace.toString(), list.space.toString());
        }
        ...
        try {
            emitSpaceEvent(list.space.toString(), "task_created", {
                task: {
                    _id: task._id,
                    title: task.title,
                    status: task.status,
        ...
        if (assignee && assignee !== createdBy) {
            ...
            enhancedNotificationService.notifyTaskUpdate(task._id.toString(), createdBy, "assignee", null, assignee).catch((error) => {
                console.error("Failed to send task assignment notification:", error);
            });
        }
        return task;
    }
    async getListTasks(listId, userId, filters) {
    ...
    async updateTask(taskId, userId, updateData) {
        ...
        if (updateData.status && updateData.status !== task.status) {
            const canTransition = await taskDependencyService.canTransitionToStatus(taskId, updateData.status);
            if (!canTransition.allowed) {
                throw new AppError(canTransition.reason || "Task is blocked by dependencies", 400);
            }
        }
        ...
        if (updateData.startDate !== undefined)
            task.startDate = updateData.startDate;
        ...
        if (updateData.assigneeId !== undefined)
            task.assignee = updateData.assigneeId;
        if (updateData.isMilestone !== undefined)
            task.isMilestone = updateData.isMilestone;
        // Enforce milestone constraint: startDate === dueDate (duration = 0)
        ...
        if (updateData.frequency)
            task.frequency = updateData.frequency;
        if (updateData.interval !== undefined)
            task.interval = updateData.interval;
        if (updateData.nextOccurrence !== undefined)
            task.nextOccurrence = updateData.nextOccurrence;
        if (updateData.recurrenceEnd !== undefined)
            task.recurrenceEnd = updateData.recurrenceEnd;
        // Validate and update custom field values if provided
        if (updateData.customFieldValues) {
            const validatedCustomFieldValues = await customFieldService.validateCustomFieldValues(updateData.customFieldValues, task.workspace.toString(), task.space.toString());
        ...
        await this.trackFieldChanges(taskId, userId, task.workspace.toString(), oldValue, {
            status: task.status,
            assignee: task.assignee,
            priority: task.priority,
        ...
        await this.emitTaskUpdateEvents(task, oldValue, userId, statusChanged, priorityChanged, assigneeChanged, oldAssignee, updateData);
        return task;
    }
    /**
     * Emit task update events and send notifications to offline users
     */
    async emitTaskUpdateEvents(task, oldValue, userId, statusChanged, priorityChanged, assigneeChanged, oldAssignee, updateData) {
        try {
            const eventData = {
                task: {
                    _id: task._id,
            ...
            if (statusChanged) {
                emitSpaceEvent(task.space.toString(), "task_updated", {
                    task: {
                        _id: task._id,
                        title: task.title,
                        status: task.status,
            ...
            if (statusChanged) {
                await enhancedNotificationService.notifyTaskUpdate(task._id.toString(), userId, "status", oldValue.status, task.status);
            }
            else if (priorityChanged) {
                await enhancedNotificationService.notifyTaskUpdate(task._id.toString(), userId, "priority", oldValue.priority, task.priority);
            }
            else if (assigneeChanged) {
                await enhancedNotificationService.notifyTaskUpdate(task._id.toString(), userId, "assignee", oldAssignee, task.assignee?.toString());
            }
            else {
                // General update
                await enhancedNotificationService.notifyTaskUpdate(task._id.toString(), userId, "general", null, null);
            }
        }
        catch (error) {
    ...
    async deleteTask(taskId, userId) {
        ...
        try {
            emitSpaceEvent(task.space.toString(), "task_deleted", {
    ...
    async createSubtask(parentTaskId, userId, data) {
        ...
        // Emit real-time event
        try {
            emitTaskEvent(parentTaskId, "subtask_created", {
                subtask: {
                    _id: subtask._id,
                    title: subtask.title,
        ...
        if (parentTask.assignee && parentTask.assignee.toString() !== userId) {
            enhancedNotificationService.notifySubtaskCreated(parentTaskId, subtask._id.toString(), userId).catch((error) => {
     ...
     */
    async addDependency(taskId, dependencyTaskId, userId) {
        ...
        if (task.dependencies.some((dep) => dep.toString() === dependencyTaskId)) {
        ...
        const hasCircularDependency = await this.checkCircularDependency(dependencyTaskId, taskId);
        if (hasCircularDependency) {
            throw new AppError("Cannot add dependency: This would create a circular dependency", 400);
        }
        // Add dependency
        ...
        try {
            ...
            emitTaskEvent(dependencyTaskId, "dependent_added", {
                task: {
                    _id: dependencyTask._id,
                    title: dependencyTask.title
                },
        ...
        if (task.assignee && task.assignee.toString() !== userId) {
            enhancedNotificationService.notifyDependencyAdded(taskId, dependencyTaskId, userId, "blocker").catch((error) => {
        ...
        if (dependencyTask.assignee && dependencyTask.assignee.toString() !== userId) {
            enhancedNotificationService.notifyDependencyAdded(dependencyTaskId, taskId, userId, "dependent").catch((error) => {
     ...
     */
    async removeDependency(taskId, dependencyTaskId, userId) {
        // Verify both tasks exist
        const [task, dependencyTask] = await Promise.all([
            Task.findOne({ _id: taskId, isDeleted: false }),
            Task.findOne({ _id: dependencyTaskId, isDeleted: false })
        ...
        // Remove dependency
        task.dependencies = task.dependencies.filter((dep) => dep.toString() !== dependencyTaskId);
        dependencyTask.dependents = dependencyTask.dependents.filter((dep) => dep.toString() !== taskId);
        await Promise.all([task.save(), dependencyTask.save()]);
        // Log activity
        await logger.logActivity({
            userId,
        ...
        // Emit real-time events
        try {
            emitTaskEvent(taskId, "dependency_removed", {
                task: {
                    _id: task._id,
                    title: task.title
                },
            ...
            emitTaskEvent(dependencyTaskId, "dependent_removed", {
                task: {
                    _id: dependencyTask._id,
                    title: dependencyTask.title
                },
     ...
     */
    async getDependencies(taskId, userId) {
        const task = await Task.findOne({
            _id: taskId,
            isDeleted: false
     ...
     */
    async getDependents(taskId, userId) {
        const task = await Task.findOne({
            _id: taskId,
            isDeleted: false
    ...
    async checkCircularDependency(startTaskId, targetTaskId, visited = new Set()) {
        ...
        for (const depId of task.dependencies) {
            const hasCircular = await this.checkCircularDependency(depId.toString(), targetTaskId, visited);
            if (hasCircular) {
                return true;
            }
        }
     ...
     */
    async updateTaskWithDependencyNotification(taskId, userId, updateData) {
        ...
        if (updateData.status) {
            ...
            if (task && task.dependents && task.dependents.length > 0) {
                for (const dependent of task.dependents) {
                    const depTask = dependent;
                    if (depTask.assignee && depTask.assignee.toString() !== userId) {
                        enhancedNotificationService.notifyDependencyStatusChanged(depTask._id.toString(), taskId, userId, updateData.status).catch((error) => {
     ...
     */
    async trackFieldChanges(taskId, userId, workspaceId, oldValues, newValues) {
     ...
     */
    hasFieldChanged(oldValue, newValue, field) {
