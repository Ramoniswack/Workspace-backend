...
class GanttService {
  ...
  async updateTaskTimeline(
    ...
    try {
      ...
      for (const dependency of dependencies) {
        const dependentTask: any = dependency.task;
        ...
        const updated = await this.updateDependentTaskDates(
          dependentTask,
          dependency.type,
          dateDelta
        );
        ...
        if (updated) {
          ...
          try {
            ...
            console.log("[Gantt] Activity logging failed (non-critical):", (error as any).message);
          }

          // Emit real-time event
          try {
            emitTaskEvent(
              dependentTask._id.toString(),
              "timeline_updated",
              {
                task: {
          ...
          const cascadeResult = await this.updateTaskTimeline(
            dependentTask._id.toString(),
            dateDelta,
            userId,
            visited
   ...
   */
  private async updateDependentTaskDates(
    dependentTask: any,
   ...
   */
  calculateDateDelta(oldDate: Date | null, newDate: Date | null): number {
    if (!oldDate || !newDate) {
      return 0;
    }
   ...
   */
  async getGanttData(spaceId: string, userId: string): Promise<any[]> {
    ...
    const ganttTasks = tasks.map((task: any) => ({
      ...
      dependencies: dependencies
        .filter((dep: any) => dep.task._id.toString() === task._id.toString())
        .map((dep: any) => ({
   ...
   */
  private calculateDuration(startDate: Date | null, dueDate: Date | null): number {
   ...
   */
  async validateTimeline(taskId: string): Promise<{
    valid: boolean;
    errors: string[];
  }> {
    const errors: string[] = [];
    ...
    for (const dependency of dependencies) {
      const predecessor: any = dependency.dependsOn;
        ...
        case "FS": // Finish-to-Start
          if (predecessor.dueDate && task.startDate) {
            if (task.startDate.getTime() < predecessor.dueDate.getTime()) {
              errors.push(
                `Task cannot start before predecessor "${predecessor.title}" finishes (FS dependency)`
              );
            }
          }
          break;
        ...
        case "SS": // Start-to-Start
          if (predecessor.startDate && task.startDate) {
            if (task.startDate.getTime() < predecessor.startDate.getTime()) {
              errors.push(
                `Task cannot start before predecessor "${predecessor.title}" starts (SS dependency)`
              );
            }
          }
          break;
        ...
        case "FF": // Finish-to-Finish
          if (predecessor.dueDate && task.dueDate) {
            if (task.dueDate.getTime() < predecessor.dueDate.getTime()) {
              errors.push(
                `Task cannot finish before predecessor "${predecessor.title}" finishes (FF dependency)`
              );
            }
          }
          break;
        ...
        case "SF": // Start-to-Finish
          if (predecessor.startDate && task.dueDate) {
            if (task.dueDate.getTime() < predecessor.startDate.getTime()) {
              errors.push(
                `Task cannot finish before predecessor "${predecessor.title}" starts (SF dependency)`
              );
            }
          }
          break;
