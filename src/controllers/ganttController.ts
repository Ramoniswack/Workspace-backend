 ...
 */
exports.updateTaskTimeline = async (req: any, res: any, next: any) => {
  try {
    ...
    if (startDate && task.startDate) {
      dateDelta = ganttService.calculateDateDelta(
        task.startDate,
        new Date(startDate)
      );
    ...
    const result = await ganttService.updateTaskTimeline(
      taskId,
      dateDelta,
      userId
    );
 ...
 */
exports.getGanttData = async (req: any, res: any, next: any) => {
  try {
 ...
 */
exports.validateTimeline = async (req: any, res: any, next: any) => {
  try {
    const { taskId } = req.params;
 ...
 */
exports.toggleMilestone = async (req: any, res: any, next: any) => {
  try {
    const { taskId } = req.params;
    const userId = req.user.id;
    ...
    const updateData: any = { isMilestone };
